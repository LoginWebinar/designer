...
interface ChildProps {
  show: boolean;
  toggleShow: any;
  useLayerData: (layerType:string,fileToUpload:File|undefined)=>void;
}


function CreateAssetLayer(props:ChildProps){ 
  ...
  async function createLayerData(){
    /**
    ...
    props.useLayerData(type,assetFile);
                ...
                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-gray-600 px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 w-full max-w-lg mx-4 sm:p-2">
                  <div className="flex flex-col gap-4">
                    <section className="mt-4">
                      <label htmlFor="type" className="block text-xs px-1.5 font-medium text-gray-200 mb-1">
                        Asset Type
                      </label>
                      <select
                        name="type"
